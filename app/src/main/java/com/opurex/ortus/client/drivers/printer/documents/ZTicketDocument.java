...
import com.opurex.ortus.client.models.PaymentDetail;
import com.opurex.ortus.client.models.PaymentMode;
import com.opurex.ortus.client.models.ZTicket;
...
public class ZTicketDocument implements PrintableDocument
{
    ...
    public boolean print(Printer printer, Context ctx) {
        ...
        printer.printLine(PrinterHelper.padAfter(ctx.getString(R.string.tkt_z_total), 10) + PrinterHelper.padBefore(priceFormat.format(this.z.getTotal()) + currencySymbol, 22));
        printer.printLine(PrinterHelper.padAfter(ctx.getString(R.string.tkt_z_subtotal), 10) + PrinterHelper.padBefore(priceFormat.format(this.z.getSubtotal()) + currencySymbol, 22));
        printer.printLine(PrinterHelper.padAfter(ctx.getString(R.string.tkt_z_taxes), 10) + PrinterHelper.padBefore(priceFormat.format(this.z.getTaxAmount()) + currencySymbol, 22));
        printer.printLine("--------------------------------");
        // Payments
        ...
        for (Integer pmId : pmt.keySet()) {
            ...
            if (mode.getLabel().length() > 20) {
                ...
                printer.printLine(PrinterHelper.padBefore(priceFormat.format(pmt.get(pmId).getTotal()) + currencySymbol, 32));
            } else {
                printer.printLine(
                        PrinterHelper.padAfter(mode.getLabel(), 20)
                        + PrinterHelper.padBefore(priceFormat.format(pmt.get(pmId).getTotal()) + currencySymbol, 12)
                );
            }
        }
        printer.printLine("--------------------------------");
        ...
        for (Double rate : this.z.getTaxBases().keySet()) {
            printer.printLine(PrinterHelper.padAfter(rateFormat.format(rate * 100) + "%", 9) + PrinterHelper.padBefore(priceFormat.format(this.z.getTaxBases().get(rate)) + currencySymbol + " / " + priceFormat.format(this.z.getTaxBases().get(rate) * rate) + currencySymbol, 23));
            }
        printer.printLine();
