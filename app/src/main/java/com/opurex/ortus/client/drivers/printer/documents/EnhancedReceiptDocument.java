 ...
 * Enhanced to include weight information for scaled products. */
public class EnhancedReceiptDocument implements PrintableDocument
{
    private Receipt r;

    public EnhancedReceiptDocument (Receipt r) {
        this.r = r;
    }
    ...
    public boolean print(Printer printer, Context ctx) {
        ...
        if (c != null) {
            ...
            printer.printLine();
            if (refill > 0.0) {
                printer.printLine(PrinterHelper.padBefore(priceFormat.format(refill) + currencySymbol, 16));
            }
