package com.opurex.ortus.client.drivers.mpop;
...
public class MPopDeviceManager implements POSDeviceManager {
    ...
    public MPopDeviceManager(DeviceManagerEventListener listener) {
        super();
        manager = new StarIoExtManager(StarIoExtManager.Type.WithBarcodeReader, OrtusPOS.getConfiguration().getPrinterModel(), "", TIMEOUT, OrtusPOS.getAppContext());
    ...
    public class PrinterCommandClassical implements MPopPrinterCommand {
        public MPopCommunication.Result sendCommand(byte[] data) {
            return MPopCommunication.sendCommands(data, OrtusPOS.getConfiguration().getPrinterModel(), "", TIMEOUT);
        }
